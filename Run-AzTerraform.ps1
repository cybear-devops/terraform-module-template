...
-BackendStorageAccountBlobStatefileName "cdo-uks-prd-test-build"
#>
...
param (
    ...
    [string]$TerraformCompliancePolicyFiles = "git:https://github.com/cybear-devops/azure-naming-convention.git//?ref=main",
    ...
    function Invoke-TerraformApply
    {
        if ($RunTerraformApply -eq $true)
        {
            try
            {
                ...
                if (Test-Path tfplan.plan)
                {
                    terraform apply tfplan.plan | Out-Host
                }
                else
                {
